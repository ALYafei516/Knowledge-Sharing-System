...
if(isset($_POST["admin_email_address"]))
{
	...
	if($total_row == 0)
	{
		$object->query = "
			...
			WHERE expert_email_address = :admin_email_address
		";
		$object->execute($data);
		...
		else
		{
			...
			{
				if($row["expert_status"] == 'Inactive')
				{
					$error = '<div class="alert alert-danger">Your account is Inactive, Contact Admin</div>';
				}
				else
				{
					if($_POST["admin_password"] == $row["expert_password"])
					{
						$_SESSION['admin_id'] = $row['expert_id'];
						$_SESSION['type'] = 'expert';
						$url = $object->base_url . 'admin/complaint.php';
					}
					else
					{
