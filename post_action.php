...
if(isset($_POST["action"]))
{
	if($_POST["action"] == 'fetch')
	{
		...
		{
			$search_query .= 'WHERE post_title LIKE "%'.$_POST["search"]["value"].'%" ';
		...
		{
			$sub_array = array();
			//$sub_array[] = '<img src="'.$row["expert_profile_image"].'" class="img-thumbnail" width="75" />';
			$sub_array[] = $row["post_title"];
			$sub_array[] = $row["post_description"];
	...
	if($_POST["action"] == 'Add')
	{
		...
		else
		{
			...
			else
			{
			    ...
			    imagepng($image, $path);
			    imagedestroy($image);
			    $expert_profile_image = $path;
			}

			if($error == '')
			{
				$data = array(
					':post_title'			=>	$object->clean_input($_POST["post_title"]),
				...
				(post_title, expert_password, expert_name, expert_profile_image, expert_phone_no, expert_address, expert_date_of_birth, expert_degree, expert_expert_in, expert_status, expert_added_on) 
				VALUES (:post_title, :expert_password, :expert_name, :expert_profile_image, :expert_phone_no, :expert_address, :expert_date_of_birth, :expert_degree, :expert_expert_in, :expert_status, :expert_added_on)
				";
